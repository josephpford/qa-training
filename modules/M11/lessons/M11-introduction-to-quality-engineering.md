...
- All password fields should mask the user's password as they enter it
- When the user clicks on the country filter, the choices should automatically be filtered to only include that country

Not all functional requirements necessarily need to be documented. Some of them can be assumed just by common sense or by following "general" software requirements, such as [Accessibility Requirements](https://www.a11yproject.com/checklist/).
...
- The search results must be returned to the customer within 25ms
- System should have the ability to scale up automatically to handle increased user load
- System should have the ability to do deployments with zero downtime for users
- System should be able to be deployed to Amazon Web Services or Microsoft Azure or Google Cloud Platform with minimal re-configuration of the underlying code
- System should be able to switch data providers easily because the organization is moving away from a hosted RDMS solution and towards a cloud-based solution, but architecture has not decided on a final solution yet.
...
The way testers communicate the presence of defects is by opening a [defect report](https://www.youtube.com/watch?v=rTmEUiWwgLs). A defect report is simply a formal way of documenting that a defect exists, what the defect is all about, how to reproduce the defect, etc. Most companies will require that all defects be entered into a Defect Tracking Tool. There are many such tools, both commercial and open source and many of them integrate with other project and documentation management tools.  
...
Agile projects follow a similar development model, but much smaller in scale. Please take time to read the [Agile manifesto](https://agilemanifesto.org/) to understand why the Agile development model was created and what it is trying to solve. Please take the time to read the [Agile principles](https://agilemanifesto.org/principles.html) as well. There are multiple development models under the Agile umbrella: Extreme Programming (XP), Kanban, Scrum, Lean. There is no "pure" agile development methodology. The basic idea behind Agile development is that "we're going to try something, and if it doesn't work, we're going to talk about it and try something else."
...
- [Functional Testing](https://en.wikipedia.org/wiki/Functional_testing)
  ...
  - [User Acceptance Testing](https://en.wikipedia.org/wiki/Acceptance_testing)
    ...
    - UAT is usually performed by a product owner or their team, but it's goal is to ensure that the software meets all of the expectations of the actual end-user. 
  ...
  - [Smoke Testing](https://en.wikipedia.org/wiki/Smoke_testing_(software))
    ...
    - Sometimes code works perfectly fine in a test environment, but once it is deployed to production, it doesn't work correctly because the data in the test environment was setup differently or perhaps there are firewalls in place in production that do not exist in the test environment.
  ...
  - [Exploratory/Ad-hoc Testing](https://www.atlassian.com/continuous-delivery/software-testing/exploratory-testing)
    ...
    - Most "Testing" is intentionally well-documented, so as to provide proof as to what has been verified as working, what has not yet been verified as working, and what has been identified as not working.
